...
class Servo : public Driver {
   public:
    ...
    bool getAngle(float& angle_deg) const;
    bool getADCValue(uint16_t& adc_value) const;
   ...
   private:
    ...
    uint32_t last_set_pulse_width_us = 0;
    float last_set_angle_deg = std::numeric_limits<float>::quiet_NaN();

    bool initTimerPWM();
    bool initADC();

    uint32_t angleToPulseWidth(float angle_deg) const;
    float adcToAngle(uint32_t adc_value) const;
};